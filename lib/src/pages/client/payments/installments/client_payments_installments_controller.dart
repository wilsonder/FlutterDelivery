...
class ClientPaymentsInstallmentsController extends GetxController {
  ...
  String identificationType = Get.arguments['identification_number'];
  ...
  //metodo para obtener el numero de cuotas de mercado pago
  void getInstallments() async {
    if (cardToken.firstSixDigits != null) {
      var result =  await mercadoPagoProvider.getInstallmets(cardToken.firstSixDigits!, total.value);
  ...
  void createPayment() async {
    ...
    //aqui validamos las diferentes respuestas que genera mercado pago al realizar una transaccion una vez finalizada, las diferentes respuiestas se encuentran el metodo badRequestProcess
    if (response.statusCode == 201) {
      ResponseApi responseApi = ResponseApi.fromJson(response.body);
      MercadoPagoPayment mercadoPagoPayment = MercadoPagoPayment.fromJson(responseApi.data);
      GetStorage().remove('shopping_bag'); //eliminar los productos de la bolsa de compras
      Get.offNamedUntil('/clients/payments/status', (route) => false, arguments: {
  ...
  void badRequestProcess(dynamic data){
    Map<String, String> paymentErrorCodeMap = {
      '3033' : 'La longitud de caracteres de la tarjeta es incorrecta',
